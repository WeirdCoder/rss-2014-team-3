...
public class VisionGUI extends JPanel implements NodeMain {
	...
	/**
	 * <p>Needed as JPanel is serializable</p>
	 **/
	...
	protected MouseInputListener mouseInputListener =
		...
		@Override public void mousePressed(MouseEvent e) {
			...
			Point2D.Double d = new Point2D.Double(startDragPoint.x - lastClickedPoint.x,
					startDragPoint.y - lastClickedPoint.y);
			System.out.println("clicked point: " + startDragPoint);
			System.out.println("  rel to last: " + d);
			System.out.println("         dist: " + startDragPoint.distance(lastClickedPoint));
	...
	public void onStart(final ConnectedNode node) {
		...
		vidSub.addMessageListener(new MessageListener<sensor_msgs.Image>() {
			...
			public void onNewMessage(sensor_msgs.Image message) {
				...
				setVisionImage(rgbData, (int)message.getWidth(), (int)message.getHeight());
			}
		}
		);
		...
		odoSub.addMessageListener(
				new MessageListener<rss_msgs.OdometryMsg>() {
					...
					public void onNewMessage(rss_msgs.OdometryMsg message) {
						if ( firstUpdate ) {
							firstUpdate = false;
							resetWorldToView(message.getX(), message.getY());
						}
						setRobotPose(message.getX(), message.getY(), message.getTheta());
					}
				}
		);
