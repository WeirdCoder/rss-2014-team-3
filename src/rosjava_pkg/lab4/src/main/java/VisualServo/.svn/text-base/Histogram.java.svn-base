...
public class Histogram {
	...
	public static void overlayHistogram(Image destination, float histogram[][]) {
		...
		for (int w=0; w < histogram.length; w++) {
			...
			int h = height-1;
			byte column[][] = new byte[height][3]; 

			// Now stack the colors
			for (;numRed > 0; numRed--) {
			    //destination.setPixel(w,h--,
			    //(byte) 0xff, (byte) 0, (byte) 0);
			    column[numRed][0] = 0x54;
			}
			for (;numGreen > 0; numGreen--) {
			    //destination.setPixel(w,h--,
			    //			(byte) 0, (byte) 0xff, (byte) 0);
			    column[numGreen][1] = 0x54;
			}
			for (;numBlue > 0; numBlue--) {
			    //destination.setPixel(w,h--,
			    //			(byte) 0, (byte) 0, (byte) 0xff);
			    column[numBlue][2] = 0x54;
			}
			
			for (int hpix=h; hpix > 0; hpix--) {
			    if ( (column[hpix][1] + column[hpix][0] + column[hpix][2]) != 0) {
				destination.setPixel(w, h--, column[hpix][0], column[hpix][1], column[hpix][2]);
			    }
			}
		}
	}
	...
	public static float[][] makeHistogram(Image source, boolean hsbHistogram) {
		...
		//working on your own solution
                int hsvVal[] =  new int[3];

                    for(int x = 0; x < width; x++){
                       for(int y = 0; y < height; y++){
                          int pixel = source.getPixel(x,y);
                          int redPixel = source.pixelRed(pixel) & ~-256;
                          int greenPixel = source.pixelGreen(pixel) & ~-256;
                          int bluePixel = source.pixelBlue(pixel) & ~-256;
			  float[] hsb = Color.RGBtoHSB(redPixel, greenPixel, bluePixel, null);
			  if (hsbHistogram) {
			      if ( Math.abs(x-width/2) < 3  && Math.abs(y-height/2) < 3) {
				  hsvVal[0] += Math.round(hsb[0]*255);
				  hsvVal[1] += Math.round(hsb[1]*255);
				  hsvVal[2] += Math.round(hsb[2]*255);
                              }
				histogram[Math.round(hsb[0]*255)][0]++;
				histogram[Math.round(hsb[1]*255)][1]++;
				histogram[Math.round(hsb[2]*255)][2]++;
                          }
                          else{
                          histogram[redPixel][0]++;
                          histogram[greenPixel][1]++;
                          histogram[bluePixel][2]++;
                          }
                       }
		       if (hsbHistogram) {
			   // System.out.println("H: " + hsvVal[0]/25 + ", S: " + hsvVal[1]/25 + ", B: " + hsvVal[2]/25);
		       }
                }
		//TODO: check hsbHistogram to toggle HSB mode instead of default RGB mode
