...
#################
    ...
    # returns: void
    # calls balancedControl with each wheel moving in opposite directions
    # (the 'flipping' of one wheel's velocity is handled in the HAL)
    def rotate(self, angVelocity):
        self.balancedControl(-1*angVelocity, angVelocity); # to increase angle with x-axis, right wheel moves forwards 
                                                           # and left moves backwares. Turning left is positive
        return

    # params: velocity: float translational velocity in m/s
    # returns: void
    # calls balancedControl with each wheel moving in same direction
    # (the 'flipping' of one wheel's velocity is handled in the HAL)
    def translate(self, velocity):
        self.balancedControl(velocity/self.WHEEL_RADIUS, velocity/self.WHEEL_RADIUS);
        return
    ...
    # param: rWheelVelocity: float ang velocity for right wheel in rad/s
    #        lWheelVelocity: float ang velocity for left wheel in rad/s
    # returns: void
    # uses PD control to maintain relative velocity between wheels
    # sends PWM messages to control wheel motors
    # (the 'flipping' of one wheel's velocity is handled in the HAL)
    def balancedControl(self, rWheelVelocity, lWheelVelocity):
#        desiredAngDiff = rWheelVelocity - lWheelVelocity;  # how much faster the right wheel should be moving
        desiredAngVelAvg = (rWheelVelocity + lWheelVelocity); # desired translational velocity
#        actualAngVelDiff = self.currentWheelAngVel[RIGHT] - self.currentWheelAngVel[LEFT]; # how much faster the right wheel is moving
        desiredAngVelErr = desiredAngVelDiff - actualAngVelDiff;  # if positive, the right wheel needs to slow down 
                                                                  #   and the left should speed up

        # TODO: in lab code - but does not make sense to me right now
        # TODO: figure out wtf is going on with this right now
        # if the right wheel is moving faster relative to the left than it should be, slow the right and speed the left
        commandLWheelAngVel = desiredAngVelAvg - (angDiffErr*self.BALANCE_GAIN);
        commandRWheelAngVel = desiredAngVelAvg + (angDiffErr*self.BALANCE_GAIN);
        # TODO
        pass
