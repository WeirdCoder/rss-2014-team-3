    ...
    for obstacle in mapList:
        ...
        if (point.getX() > obstacle.getMinX) and \
            ...
            for i in range(len(obsPointList)):
                if distancePointToLine(point, obsPointList[i], obsPointList[(i+1)%len(obsPointList)]) < SONAR_UNCERTAINTY:
                    return False
...
def makeObstacle():
    ...
    obstacle = createRectangle(enddpoint1, endpoint2, width)
    sendObstacleMsg(obstacle)
    pointList = []
    return
